...
// PWM output 
#define pwmPin1A  12 //OC1A
#define pwmPin1B  11 //OC1B
...
ISR(TIMER1_OVF_vect){
  ...
  static uint8_t  index1r      = 0 , index1l      = 0;
  static uint16_t phaseAccum1r = 0 , phaseAccum1l = 0;
  ...
  if (stimulusLeft) 
    osc1B = pgm_read_byte( &sineTable[index1l] );
}

// TIMER3 overflow 62.5KHz.
ISR(TIMER3_OVF_vect){
  static uint8_t osc3A         = 0, osc3B       = 0;
  static uint8_t index3r       = 0, index3l     = 0;
  static uint16_t phaseAccum3r = 0, phaseAccum3l= 0;
  ...
  // Read oscillator value for next interrupt
  if (feedbackLeft) 
    osc3A = pgm_read_byte( &sineTable[index3l] );
  if (feedbackRight) 
...
void initOneToneTimer(char tipo){
	if (tipo=='s'){//Stimulus 
		pinMode(pwmPin1A, OUTPUT);
		pinMode(pwmPin1B, OUTPUT);
		TCCR1A = _BV(COM1A1) | _BV(COM1B1) | _BV(WGM10);
...
the signals will come out*/
  ...
  if(right){
      ...
      stimulusRight    = right;}
    if(left){
      phaseIncrement1l = setFrequency(freq);
      stimulusLeft     = left;}
    }
  if (tipo=='f'){//Feedback
    TIMSK3         |= _BV(TOIE3);
    if(right){
      phaseIncrement3r = setFrequency(freq);
      feedbackRight    = right;}
    if(left){ 
      phaseIncrement3l = setFrequency(freq);
      feedbackLeft     = left;}
  }
  if (tipo=='n'){//Noise
    TIMSK2         |= _BV(TOIE2); 
...
void tickOff(char tipo,uint16_t freq, boolean left, boolean right) {
/* Disable the timerOverflowInterrupt based on 'tipo'*/
  if (tipo=='s'){//Stimulus
    if(right){ 
      phaseIncrement1r = setFrequency(1);
      stimulusRight    = ~right;}
    if(left){ 
      phaseIncrement1l = setFrequency(1);
      stimulusLeft     = ~left;}
  }
  if (tipo=='f'){
    if(right){
      feedbackRight    = right;
      phaseIncrement3r = setFrequency(1);}
    if(left){
      feedbackLeft     = left;
      phaseIncrement3l = setFrequency(1);}
  }
  if (tipo=='n'){
    TIMSK2 &= ~_BV(TOIE2);
    if(right)
