...
ISR(TIMER3_OVF_vect){
  ...
  if (feedbackLeft) 
    ...
    osc3A = pgm_read_byte( &sineTable3[1] );
  if (feedbackRight) 
    osc3B = pgm_read_byte( &sineTable[index3r] );
  else
    osc3B = pgm_read_byte( &sineTable3[1] );
}
...
/* Disable the timerOverflowInterrupt based on 'tipo'*/
  if (tipo=='s'){//Stimulus
    if(right){ 
//      phaseIncrement1r = setFrequency(3);
      stimulusRight    = false;}//!right;}
    if(left){ 
//      phaseIncrement1l = setFrequency(3);
