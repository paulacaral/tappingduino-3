...
int tone_freq3 = 1800;//(C6) //1046.5; // defines the frequency (i.e., pitch) of the tone (in Hz)
...
static const uint8_t  sineTable4[] PROGMEM =
{
...
// PWM output (OCR3A)
int pwmPin2A = 10; //Channel A OC3A. Timer3 controls pins 2,3 and 5
int pwmPin2B = 9; //Channel B OC3B
...
ISR(TIMER1_OVF_vect)
{
  ...
  osc1A = pgm_read_byte( &sineTable4[index1] );
  
  if (stimulusLeft==true) 
  osc1B = pgm_read_byte( &sineTable1[index1] );
  else
  osc1B = pgm_read_byte( &sineTable4[index1] );
...
OCR3B = osc3B;// timer 3 - pin 2 - channel B - Left
  ...
  osc3A = pgm_read_byte( &sineTable4[index3] );
  
  if (feedbackRight==true) 
  osc3B = pgm_read_byte( &sineTable1[index3] );
  else
  osc3B = pgm_read_byte( &sineTable4[index3] );
}
...
OCR2B = osc2B;// timer 2 - pin 9 - channel B - Left
  ...
  osc2A = pgm_read_byte( &sineTable4[index2] );
  
  if (NoiseRight==true) 
  osc2B = pgm_read_byte( &sineTable1[index2] );
  else
  osc2B = pgm_read_byte( &sineTable4[index2] );
}*/
...
void setFrequency2( uint16_t frequency2 )
{
  ...
  phaseIncrement2 = phaseIncr642 >> 8;
}
...
void tickOff2() {
  NoiseLeft=false;
  NoiseRight=false;
  setFrequency2( .05 ); // so that you practically don't hear anything (well below perceptual limit of humans)
}
